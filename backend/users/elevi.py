...
# --- 2. POST: Adaugă un elev (Doar Copii) ---
...
# --- 3. PATCH: Modifică un elev (Funcționează pt Copii ȘI Adulți) ---
...
def update_student(elev_id):
    ...
    try:
        ...
        # Verificăm dacă ID-ul pare a fi numeric (adică Sportiv Adult / User)
        is_probably_user = str(elev_id).isdigit()

        # --- ÎNCERCARE 1: Dacă e ID numeric, actualizăm tabela UTILIZATORI ---
        if is_probably_user:
            fields_u = []
            values_u = []
            ...
            if fields_u:
                ...
                try:
                    ...
                    # Dacă dă eroare, facem rollback și încercăm la copii (just in case)
                    print(f"Eroare update user (dar continuam): {e}")
                    con.rollback()
                    cur = con.cursor()

        # --- ÎNCERCARE 2: Dacă nu am actualizat user, actualizăm tabela COPII ---
...
def delete_student(elev_id):
    ...
    try:
        ...
        # 2. Sportiv (User) - Doar îl scoatem din grupă
        if not deleted:
            try:
